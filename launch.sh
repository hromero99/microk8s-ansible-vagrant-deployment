...
register_node(){
    ...
    node_join=$(/opt/vagrant ssh --command "sudo microk8s.add-node | grep 192.168.50" kubernetes-master)
    echo "Adding node $1 to kubernetes cluster"
    echo "Using $node_join"
    /opt/vagrant ssh --command="sudo $node_join" $1
}

get_nodes(){
  command=$(/opt/vagrant ssh --command="sudo microk8s.kubectl get nodes --show-labels" kubernetes-master)
...
cluster_status(){
  ...
  for node in node-1 node-2 node-3 kubernetes-master; do
    status=$(/opt/vagrant ssh --command="sudo microk8s.status" $node | grep 'microk8s is*') >/dev/null
    echo "[?] $node -- $status"
  done
}
...
add_label_to_node(){
  ...
  result=$(/opt/vagrant ssh --command="sudo microk8s.kubectl label nodes $1 $2" kubernetes-master )
...
delete_vm_node(){
  ...
  result=$(/opt/vagrant destroy --force $1)
  echo "$result"
}

create_vm_node(){
  echo "[!] Going to create $1"
  result=$(/opt/vagrant up $1)
  echo "$result"
}
